        ...
        try:
            ...
            for size in sizes:
                ...
                if size <= 512:  # 2x 버전도 생성
                    resized_img.save(f"{iconset_dir}/icon_{size//2}x{size//2}@2x.png")
...
def build_mac_app():
    print("\n" + "="*80)
    print("macOS 애플리케이션 빌드 시작...")
    print("="*80 + "\n")
    
    # 관리자 권한 확인
    if not is_admin():
        print("경고: 관리자 권한으로 실행되지 않았습니다. 파일 접근 문제가 발생할 수 있습니다.")
        print("sudo를 사용하여 다시 실행하는 것을 권장합니다.")
    ...
    kill_processes_by_name([
        "DealerDesk",
        "uvicorn",
        "Python"  # macOS에서는 좀 더 일반적인 이름으로 지정
    ])
    ...
    pyinstaller_cmd = [
        ...
        "--icon=app_icon.icns",  # macOS 아이콘 파일
        "--workpath=" + work_dir,  # 작업 디렉토리 명시적 지정
        "--noconfirm",  # 확인 없이 진행
        "--clean",  # 빌드 전 정리
        "--log-level=DEBUG",  # 디버그 로그 활성화
        "--osx-bundle-identifier=com.dealerdesk.app",  # macOS 번들 식별자
        "--add-data=app_icon.icns:.",  # 아이콘 파일을 리소스로 포함
    ]
    ...
    # 배포 후 처리
    print("빌드 후 처리 중...")
    app_path = os.path.join("dist", "DealerDesk.app")
    
    # README 생성
    create_readme_file(app_path)
    
    # _internal 디렉토리 확인
    check_internal_dir(app_path)
    
    print("\n" + "="*80)
    print("빌드 완료!")
    print(f"애플리케이션 위치: {app_path}")
    print("="*80)
...
def manual_copy_files():
    ...
    internal_dir = os.path.join(resources_dir, "_internal")
    
    for d in [app_path, contents_dir, resources_dir, internal_dir]:
...
def check_internal_dir(app_path):
    """_internal 디렉토리가 존재하는지 확인하고 없으면 생성"""
    resources_dir = os.path.join(app_path, "Contents", "Resources") if app_path.endswith(".app") else app_path
    internal_dir = os.path.join(resources_dir, "_internal")
...
def create_readme_file(app_path):
    """배포 디렉토리에 README 파일 생성"""
    readme_path = os.path.join(os.path.dirname(app_path), "README.txt")
    try:
        with open(readme_path, "w", encoding="utf-8") as f:
            f.write("""딜러 데스크 트레이 애플리케이션 (macOS 버전)

사용 방법:
1. DealerDesk.app 파일을 실행합니다.
2. 메뉴바에 아이콘이 나타납니다.
3. 메뉴바 아이콘을 클릭하면 메뉴가 표시됩니다.
...
문제 해결:
- 권한 오류가 발생한다면 '시스템 환경설정 > 보안 및 개인 정보 보호'에서 앱 실행을 허용해주세요.
- 서버 시작 문제는 로그 파일을 확인하세요.
- '개발자를 확인할 수 없음' 메시지가 나타나면 컨트롤 키를 누른 상태에서 앱을 클릭하고 '열기'를 선택하세요.
